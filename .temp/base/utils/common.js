...
import Taro, { getStorageSync as _getStorageSync, setStorage as _setStorage, removeStorageSync as _removeStorageSync } from "@tarojs/taro-h5";
...
export const setKeywordInHistory = keyword => {
  const keywordsList = _getStorageSync('keywordsList') || [];
  ...
  _setStorage({ key: 'keywordsList', data: keywordsList });
};

// 获取搜索关键字
export const getKeywordInHistory = () => {
  return _getStorageSync('keywordsList');
};

// 清除搜索关键字
export const clearKeywordInHistory = () => {
  _removeStorageSync('keywordsList');
};
