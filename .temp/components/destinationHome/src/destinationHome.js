...
const ContainerWH = 1;
const marginBu = 0.15000000000000002;

class DestinationHome extends Taro.Component {
  render() {
    ...
    return <div className={"ContainerDiv"}>
      {imgList && imgList.map((item, index) => {
        ...
        return <div className={"ComponentDestinationHomeImageList"} style={{ paddingBottom: index === 1 ? "0.2rem" : "0" }} key={`imgList_${index}`}>
            {imgDom}
            {endDom}
          </div>;
      })}
